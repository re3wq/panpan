...
async function deviceTestingInit() {
  ...
  $('#start-test-btn').on('click', function() {
    ...
    $('#device-testing').show();
    // startCameraTesting();
      if(is_video_after==false){
          hasCameraConnect=undefined;
      }
      if(hasCameraConnect==undefined){
          if(isSafari){
...
function deviceDialogInit() {
  if (!localStorage.getItem('txy_device_testing')) {
    ...
    if (!(hasVoiceConnect && hasMicConnect)) {
 ...
 * 弹窗-设备连接检查(订单专用)
 */
function startDeviceConnect_order() {
    //禁止滑动
...
function showTestingReport() {
  ...
  console.log(cameraTestingResult);

    if(!cameraTestingResult.device){
