    ...
    $.fn.extend({
        select: function (options) {
            ...
            var defaults = {
                ...
                fontHoverColor: "black",   //移动选择时，每一行的字体hover颜色
            ...
            $this.find("option").each(function (index, element) {
                var $li = $('<li val=' + $(element).val() + '>' +'<div style="padding-top:8px;padding-bottom:8px;" class="lt_label_wrap"><input name="selectTip" type="checkbox"/><span>'+ $(element).text() + '</span></div></li>');
                if ($(element).prop("selected")) {
                    $li.addClass("list_current");
                    //$This.find(".select_content").text($(element).text());
                     $This.find(".select_content").html($(element).text());
				}
            ...
            function Search(self) {
                ...
                $This.find(".select_list_ul li").not(".no_result").each(function (index, element) {
					...
					if($(element).find('span').text().trim()=='标签搜索'){
						$(element).hide();
					}
                });
            ...
            $This.find(".select_arrow,.select_content,.select_arrow_after").click(function () {
                ...
                if ($This.find(".select_list").hasClass("list_open")) {
					...
					if($(".pan-teaching-list-selectw").index($This.parents(".pan-teaching-list-selectw"))==0){
						if($(".pan-teaching-list-selectw").eq(1).find(".select_arrow_after").hasClass("cast_rotate")){
							$(".pan-teaching-list-selectw").eq(1).find(".select_arrow_after").css("margin-top", "-1px");
						    $(".pan-teaching-list-selectw").eq(1).find(".select_arrow").removeClass("cast_rotate");
							$(".pan-teaching-list-selectw").eq(1).find(".select_arrow_after").removeClass("cast_rotate");
						}
						$(".pan-teaching-list-selectw").eq(1).find(".select_list").removeClass("list_open").animate({ "height": "0px" }, 200, function () {
							$(".pan-teaching-list-selectw").eq(1).find(".select_list").hide();
						});
						
					}else{
						
						if($(".pan-teaching-list-selectw").eq(0).find(".select_arrow_after").hasClass("cast_rotate")){
						$(".pan-teaching-list-selectw").eq(0).find(".select_arrow_after").css("margin-top", "-1px");
						$(".pan-teaching-list-selectw").eq(0).find(".select_arrow").removeClass("cast_rotate");
						$(".pan-teaching-list-selectw").eq(0).find(".select_arrow_after").removeClass("cast_rotate");
						}
						$(".pan-teaching-list-selectw").eq(0).find(".select_list").removeClass("list_open").animate({ "height": "0px" }, 200, function () {
							$(".pan-teaching-list-selectw").eq(0).find(".select_list").hide();
						});
					}
				}
			});
            ...
            $This.find(".select_list_body").delegate("li", "click", function (e) {
                ...
                //console.log("xcccc:",$(this).find('input:checkbox').is(":checked"))
                ...
                if(e.target.nodeName!='INPUT'){
                    if($(this).find('input:checkbox').is(":checked")){
                        ...
                        //console.log("xcccc2:",$(this).find('input:checkbox').is(":checked"))
                    }
                }
				...
				var select_content_t = '';
                var select_content_v = '';
				//获取选中的checkbox内容
				$("input:checkbox[name='selectTip']:checked").each(function(){
					//alert($(this).parent().find('span').text());
					select_content_t += $(this).parent().find('span').text()+',';
                    //console.log($(this).parent().parent().attr('val'));
                    select_content_v += "'" + $(this).parent().parent().attr('val') + "'" + ',';
				});
				
				select_content_t = select_content_t.substring(0,select_content_t.length-1);
                select_content_v = select_content_v.substring(0,select_content_v.length-1);
                ...
                //$This.find(".select_content").text(select_content_t);
                $('.select-tips-list').text(select_content_t);	    //  ##  添加标签
                //console.log(select_content_v);
                get_tag_teachers(select_content_v); // 动态获取符合该标签的老师
