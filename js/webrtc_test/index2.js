...
var text = {
    ...
    MicTest : "마이크 기능 테스트",
    Camera : "카메라 기능",
    CameraTest : "카메라 기능 테스트",
    Support : "해상도 테스트",
    SupportTest : "해상도 테스트",
    Connection : "연결성 테스트",
    ConnectionTest : "연결성 테스트",
    Browser : "인터넷 브라우저"
};
init();
...
function startBrowserTest(){
    $("#browser-detail")[0].innerText = "";
    var titleText = null;
    $("#browser-title")[0].innerText = '인터넷 브라우저 실패 !!!';
    ...
    for(var a in isMobile){
        if( isMobile[a]() ){
            ...
            if( a === 'Android' && version && version < 43600 ){
                $("#browser-title2").css("background", "#ffe2db");
                $("#browser-title").css("color", "#ff5026");
                titleText =  "TBS (version:"+ version + ") 실패 !!!";
            }
            else if(!isWebRTCSupported || (!isMobile.safari() && isMobile.iOS())) {
                $("#browser-title2").css("background", "#ffe2db");
                $("#browser-title").css("color", "#ff5026");
                titleText = a + "인터넷 브라우저 실패 !!!";
                onVideoTestDone(-1);
                onSupportTestDone(-1);
            }else{
                if( isMobile.safari() && isMobile.iOS()  ){
                    ...
                    if(matches && matches[1]>=11 && (matches[2]>=1 || matches[3] >= 3) ){
                        ...
                        titleText =  matches[0] + " 인터넷 브라우저 지원 !!!";
                    }else{
                        $("#browser-title2").css("background", "#ffe2db");
                        $("#browser-title").css("color", "#ff5026");
                        titleText =  matches[0] + "  실패 !!!";
                    }
                }else{
                    $("#browser-title2").css("background", "#e9f2d6");
                    $("#browser-title").css("color", "#339900");
                    titleText =  a + " 인터넷 브라우저 지원 !!!";
                }
            }
