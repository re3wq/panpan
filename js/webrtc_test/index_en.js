...
function checkTBSVersion(ua) {
    ...
    for (var i = 0; i < list.length; i++) {
        ...
        if (item.indexOf("TBS") !== -1 || item.indexOf("tbs") !== -1) {
            ...
            if (version <= 43600) {
                alert("Your IBS version number is too low to support WebRTC, please upgrade");
            }
        }
    }
}
...
var text = {
    Mic : "Audio Input",
    MicTest : "Audio Input Detection",
    Camera : "Video Input",
    CameraTest : "Video Input Detection",
    Support : "Screen resolution",
    SupportTest : "Screen resolution test",
...
function startBrowserTest(){
    $("#browser-detail")[0].innerText = "";
    var titleText = null;
    $("#browser-title")[0].innerText = 'Current browser is not supported !!!';
    ...
    for(var a in isMobile){
        if( isMobile[a]() ){
            ...
            if( a === 'Android' && version && version < 43600 ){
                $("#browser-title2").css("background", "#ffe2db");
                $("#browser-title").css("color", "#ff5026");
                titleText =  "Current browser is not supported !!!";
            }
            else if(!isWebRTCSupported || (!isMobile.safari() && isMobile.iOS())) {
                $("#browser-title2").css("background", "#ffe2db");
                $("#browser-title").css("color", "#ff5026");
                titleText = a + "Current browser is not supported !!!";
                onVideoTestDone(-1);
                onSupportTestDone(-1);
            }else{
                if( isMobile.safari() && isMobile.iOS()  ){
                    ...
                    if(matches && matches[1]>=11 && (matches[2]>=1 || matches[3] >= 3) ){
                        ...
                        titleText =  matches[0] + " Current browser is supported!!!";
                    }else{
                        $("#browser-title2").css("background", "#ffe2db");
                        $("#browser-title").css("color", "#ff5026");
                        titleText =  matches[0] + "  Not supported !!!";
                    }
                }else{
                    $("#browser-title2").css("background", "#e9f2d6");
                    $("#browser-title").css("color", "#339900");
                    titleText =  a + " Current browser is supported!!!";
                }
            }
    ...
    checkH264Support(function(encode, decode){
        ...
        if( !encode || !decode ){
            ...
            if( !encode ){
                titleText +=" (H264: Encoding is not supported)"
            }
            if( !decode ){
                titleText +=" (H264: Encoding is not supported)"
            }
        ...
        if( !isMobileBrowser){
            if(isWebRTCSupported){
                titleText =  "Current browser is supported!!!";
                $("#browser-title2").css("background", "#e9f2d6");
                $("#browser-title").css("color", "#339900");
                $("#browser-title")[0].innerText = titleText;
            }else{
